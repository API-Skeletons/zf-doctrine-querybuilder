...
``` console
$ php composer.phar require "zfcampus/zf-doctrine-querybuilder-filter:~1.0-dev@dev"
```

Once installed, add `ZF\Doctrine\QueryBuilder\Filter` to your list of modules inside
...
The `$filters` array is intended to come from the request.
...
Each query definition requires at a minimum a 'type' and a 'field'.  Each query may also specify a
'where' which can be either 'and' or 'or'.  Embedded logic such as and(x or y) is supported through
AndX and OrX query types.
...
To enable it, add this to your configuration (e.g. `config/autoload/global.php`).  There is also an
optional On filter used to join two columns (e.g. `row.id = other.id`):
